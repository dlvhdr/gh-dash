...
1. [`stack`] with a width of 7 columns, hidden by default.
...
| Property | Type | Default                                                          |
| :------- | :--- | :--------------------------------------------------------------- |
| `stack`  | yaml | <Code code={`width: 7\nhidden: true`} lang="yaml" frame="none"/> |

This column displays the PR's position in its stack, counting from the branch
the stack is based on, like `2/3`. PRs that aren't part of a stack leave the
column blank.

Stacks are read from GitHub, so they show up whether they were created with
`gh stack` or in the web UI. The selected PR's full stack is shown in the
sidebar's `Stack` tab.
