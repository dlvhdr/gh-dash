...
// WithWidth sets the width of the carousel.
func WithEdgeMarkers() Option {
	return func(m *Model) {
		m.showEdgeMarkers = true
	}
}

// WithWidth sets the width of the carousel.
func WithSeparators() Option {
	return func(m *Model) {
...
func (m *Model) UpdateSize() {
	...
	if m.showEdgeMarkers && lastLeft != 0 {
		leftMarker = m.styles.OverflowIndicator.Render("<")
		l -= lipgloss.Width(leftMarker)
	}
	if m.showEdgeMarkers && lastRight != len(m.items)-1 {
		rightMarker = m.styles.OverflowIndicator.Render(">")
		l -= lipgloss.Width(rightMarker)
	}

	itemsContent = lipgloss.NewStyle().MaxWidth(l).Render(itemsContent)

	m.content = lipgloss.NewStyle().Height(m.height).Render(
		lipgloss.JoinHorizontal(lipgloss.Center, leftMarker, itemsContent, rightMarker))
}
...
func (m *Model) SetCursor(n int) {
	...
	log.Error("carousel - start", "cursor", m.cursor)
	m.UpdateSize()
}
...
func (m *Model) renderItem(itemID int, width int) string {
	var item string
	if itemID == m.cursor {
		item = m.styles.Selected.Render(m.items[itemID])
	} else {
		item = ansi.Truncate(m.styles.Item.Render(m.items[itemID]), width, "…")
	}

	if m.showSeparators && itemID != len(m.items)-1 {
		return lipgloss.JoinHorizontal(lipgloss.Center, item, m.styles.Separator.Render("|"))
	}
