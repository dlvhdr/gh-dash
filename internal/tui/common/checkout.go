...
func checkoutRegular(params CheckoutParams, repoPath string) (tea.Cmd, error) {
	...
	return tea.Batch(startCmd, func() tea.Msg {
		...
		err := RunCommandFunc("gh", []string{"pr", "checkout", fmt.Sprint(params.PRNumber)}, expandedPath)
...
func checkoutWorktree(params CheckoutParams, repoRoot string, wtParent string) (tea.Cmd, error) {
	...
	task := CheckoutTask{
		...
		FinishedText: fmt.Sprintf("PR #%d checked out into worktree (path copied to clipboard)", params.PRNumber),
	}
	startCmd := params.StartTask(task)
	...
	return tea.Batch(startCmd, func() tea.Msg {
		...
		if err := os.MkdirAll(expandedParent, 0o755); err != nil {
			return constants.TaskFinishedMsg{TaskId: taskId, Err: fmt.Errorf("create worktree directory: %w", err)}
		}

		// Create a detached worktree
		err := RunCommandFunc("git", []string{"worktree", "add", "--detach", wtPath, "HEAD"}, expandedRoot)
		if err != nil {
			return constants.TaskFinishedMsg{TaskId: taskId, Err: fmt.Errorf("git worktree add: %w", err)}
		}
		...
		if err != nil {
			...
			return constants.TaskFinishedMsg{TaskId: taskId, Err: fmt.Errorf("gh pr checkout: %w", err)}
		}
